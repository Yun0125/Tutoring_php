...
class BoardDAO {
	...
	public function insertMsg($title, $writer, $content) {
		try {
			$sql = "insert into review(title, writer, content) values(:title, :writer, :content)";
	...
	public function increaseHits($num) {
		try {
			$sql = "update review set hits=hits+1 where num=:num";
			$pstmt = $this->db->prepare($sql);
	...
	public function updateMsg($num, $title, $writer, $content) {
		try {
			$sql = "update review set title=:t, writer=:w, content=:c where num=:n"; 
	...
	try {
			$sql = "select * from review order by num desc limit :startRecord, :count";	
			$pstmt = $this->db->prepare($sql);	
	...
	public function getTotalCount() {
		try {
			$sql = "select count(*) as totalCount from review";
			$pstmt = $this->db->prepare($sql);
			$pstmt->execute();
